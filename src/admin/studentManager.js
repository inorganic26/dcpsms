...
export const studentManager = {
    ...
    cacheElements() {
        this.elements = {
            ...
            pagination: document.getElementById('admin-student-pagination'),
            ...
            editStudentClassSelect: document.getElementById('admin-edit-student-class-select'),
        };
    },
    ...
    async createStudent() {
        ...
        if (!name || !phone) { showToast("이름과 전화번호를 입력하세요.", true); return; }
        if (phone.length < 4) { showToast("전화번호는 최소 4자리 이상이어야 합니다.", true); return; }

        try {
            showToast("학생 계정을 생성 중입니다...", false);
            const functions = getFunctions(app, 'asia-northeast3');
            const createAccountFn = httpsCallable(functions, 'createStudentAccount');
            const result = await createAccountFn({ name, phone, parentPhone });
    ...
    openEditModal(student) {
        ...
        if (classSelect) {
            ...
            if (classes.length === 0) {
                ...
                classes.forEach(c => {
                    ...
                    label.innerHTML = `<input type="checkbox" value="${c.id}" class="class-checkbox w-4 h-4 text-indigo-600 rounded" ${isChecked ? 'checked' : ''}><span>${c.name}</span>`;
        ...
        modal.style.display = 'flex'; // tailwind 'flex' class 대신 직접 스타일 제어 권장
    },
    ...
    async updateStudent() {
        ...
        } else if (this.elements.editStudentClassSelect?.value) {
            newClassIds = [this.elements.editStudentClassSelect.value];
        }

        if (!newName || !newPhone) { showToast("이름과 전화번호를 입력하세요.", true); return; }
    ...
    renderStudentList() {
        ...
        if (searchInput && searchInput.value.trim() !== "") {
            ...
            students = students.filter(s => s.name.toLowerCase().includes(keyword) || (s.phone && s.phone.includes(keyword)));
        }
        ...
        currentStudents.forEach(student => {
            ...
            if (studentClassIds.length === 0) {
                ...
                classBadgeHtml = studentClassIds.map(cid => {
                    ...
                    if (!cls) return ""; 
                    const colorClass = cls.classType === 'live-lecture' ? "bg-indigo-50 text-indigo-700" : "bg-emerald-50 text-emerald-700";
            ...
            tr.innerHTML = `
                ...
                <td class="p-3">${classBadgeHtml}</td>
                <td class="p-3 text-right">
                    <button class="edit-btn text-slate-400 hover:text-blue-600 p-2 rounded-full hover:bg-blue-50 transition"><span class="material-icons text-sm">edit</span></button>
                    <button class="delete-btn text-slate-400 hover:text-red-600 p-2 rounded-full hover:bg-red-50 transition"><span class="material-icons text-sm">delete</span></button>
                </td>
            `;
    ...
    renderPagination(totalItems, totalPages) {
        ...
        const createBtn = (text, onClick, disabled = false, isActive = false) => {
            ...
            if (String(text).includes('material-icons')) { 
                ...
                btn.className = `w-8 h-8 flex items-center justify-center rounded text-sm font-bold transition ${isActive ? 'bg-indigo-600 text-white shadow-md' : 'bg-white text-slate-600 hover:bg-slate-50 border border-slate-200'}`;
            }
            return btn;
        };

        paginationContainer.appendChild(createBtn('<span class="material-icons text-sm">chevron_left</span>', () => { this.state.currentPage--; this.renderStudentList(); }, this.state.currentPage === 1));
        ...
        for (let i = startPage; i <= endPage; i++) {
            paginationContainer.appendChild(createBtn(i, () => { this.state.currentPage = i; this.renderStudentList(); }, false, i === this.state.currentPage));
        }

        paginationContainer.appendChild(createBtn('<span class="material-icons text-sm">chevron_right</span>', () => { this.state.currentPage++; this.renderStudentList(); }, this.state.currentPage === totalPages));
    },
